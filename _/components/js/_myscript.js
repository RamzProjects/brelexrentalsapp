$(function() {
	...
	$("#location a:contains('Location')").parent().addClass('active');
	...
	*/
	...
	$('img.marquee_panel_photo').each(function(index){
		...
		var photoHeight = $('.marquee_container').height();
		var photoPosition = index * photoWidth;
		$('.marquee_photos').append('<img class="marquee_photo" style="left:'+ photoPosition +'" src="'+ $(this).attr('src') +'" alt="'+ $(this).attr('alt') +'" width="'+ photoWidth +'" height="'+ photoHeight +'" />');
	...
	$('.marquee_nav a.marquee_nav_item').click(function(){
		...
		$('.marquee_photos').animate({left: newPhotoPosition}, 1000).fadeIn(200);
		$('.marquee_caption').animate({top: '500px'}, 500, function(){
